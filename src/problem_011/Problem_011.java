...
//	System.out.println(largestProductInAGrid(generateGrid(cells)));
  ...
  private static void other() {
	int[][] grid = {
	    { 8, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12, 50,
	        77, 91, 8 },
	    ...
	    { 32, 98, 81, 28, 64, 23, 67, 10, 26, 38, 40, 67, 59, 54, 70, 66, 18,
	        38, 64, 70 },
	    { 67, 26, 20, 68, 02, 62, 12, 20, 95, 63, 94, 39, 63, 8, 40, 91, 66,
	        49, 94, 21 },
	    { 24, 55, 58, 05, 66, 73, 99, 26, 97, 17, 78, 78, 96, 83, 14, 88, 34,
	        89, 63, 72 },
	    { 21, 36, 23, 9, 75, 00, 76, 44, 20, 45, 35, 14, 00, 61, 33, 97, 34,
	        31, 33, 95 },
	    { 78, 17, 53, 28, 22, 75, 31, 67, 15, 94, 03, 80, 04, 62, 16, 14, 9,
	        53, 56, 92 },
	    ...
	    { 19, 80, 81, 68, 05, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17, 77, 04,
	        89, 55, 40 },
	    { 04, 52, 8, 83, 97, 35, 99, 16, 07, 97, 57, 32, 16, 26, 26, 79, 33,
	        27, 98, 66 },
	    { 88, 36, 68, 87, 57, 62, 20, 72, 03, 46, 33, 67, 46, 55, 12, 32, 63,
	        93, 53, 69 },
	    { 04, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18, 8, 46, 29, 32, 40,
	        62, 76, 36 },
	...
	for (int y = 0; y < 20; y++) {
	  for (int x = 0; x < 20; x++) {
		...
		try {
		  ...
		  if (grid[y][x] * grid[y][x + 1] * grid[y][x + 2] * grid[y][x + 3] > biggestSum) { // right

			biggestSum = grid[y][x] * grid[y][x + 1] * grid[y][x + 2]
			             * grid[y][x + 3];

		  }

		  if (grid[y][x] * grid[y + 1][x] * grid[y + 2][x] * grid[y + 3][x] > biggestSum) { // down

			biggestSum = grid[y][x] * grid[y + 1][x] * grid[y + 2][x]
			             * grid[y + 3][x];

		  }

		  if (grid[y][x] * grid[y + 1][x + 1] * grid[y + 2][x + 2]
			  * grid[y + 3][x + 3] > biggestSum) { // diag right

			biggestSum = grid[y][x] * grid[y + 1][x + 1] * grid[y + 2][x + 2]
			             * grid[y + 3][x + 3];

		  }

		  if (grid[y][x] * grid[y + 1][x - 1] * grid[y + 2][x - 2]
			  * grid[y + 3][x - 3] > biggestSum) { // diag left
